...
import { deleteSubscriptionAction } from '../[id]/actions';

export function DeleteSubscriptionButton({ subscriptionId }: { subscriptionId: string }) {
  const [loading, setLoading] = useState(false);

  const handleDelete = async () => {
    if (!window.confirm('क्या आप सच में इस Subscription को डिलीट करना चाहते हैं? इससे सारा संबंधित डेटा भी डिलीट हो जाएगा।')) {
      return;
    }
    setLoading(true);
    try {
  ...
  return (
    ...
    >
      🗑️ {loading ? 'Deleting...' : 'Delete Subscription'}
    </button>
  );
}
