...
-- ---------------------------------------------------------------------
-- 4.2b  zoho_document_pdf
--       Cached PDF bytes of Zoho estimates/invoices. Fetched on demand
--       (GET /{estimates|invoices}/{id}?accept=pdf), stored so repeat
--       "View PDF" opens cost 0 Zoho calls. Invalidated by webhooks.
-- ---------------------------------------------------------------------
CREATE TABLE zoho_document_pdf (
    id                  UUID          PRIMARY KEY DEFAULT gen_random_uuid(),
    organization_id     UUID          NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
    entity_type         VARCHAR(20)   NOT NULL,
    zoho_doc_id         VARCHAR(80)   NOT NULL,
    zoho_doc_number     VARCHAR(80),
    ...
    CONSTRAINT uq_zoho_document_pdf UNIQUE (organization_id, entity_type, zoho_doc_id),
    CONSTRAINT chk_zoho_document_pdf_entity_type CHECK (
        entity_type IN ('estimate', 'invoice')
    )
);

CREATE INDEX idx_zoho_document_pdf_org_type ON zoho_document_pdf(organization_id, entity_type);
...
CREATE TABLE quick_quotes (
    ...
    zoho_invoice_status             VARCHAR(30),   -- live converted-invoice status (draft|sent|paid|overdue|void), synced on demand
    pushed_to_zoho_at               TIMESTAMPTZ,
    subscription_decision           VARCHAR(20),   -- convert-time choice: create_now|later|never (NULL = pre-feature, treated as later)
...
CREATE TABLE quick_quote_items (
    ...
    primary_domain          VARCHAR(255),
    domain_list             JSONB,         -- bulk-domains line: [{domain, qty}] → one subscription per domain at convert
    renewed_subscription_id UUID          REFERENCES subscriptions(id) ON DELETE SET NULL,
...
CREATE INDEX idx_qqi_renewed_sub   ON quick_quote_items(renewed_subscription_id) WHERE renewed_subscription_id IS NOT NULL;
...
CREATE TABLE subscriptions (
    ...
    -- Billing currency (prices above are in this currency, matching the customer's
    -- Zoho invoices). exchange_rate = value of 1 unit of `currency` in the org base
    -- currency (INR) at import time → base-equivalent = amount * exchange_rate.
    currency                    VARCHAR(10)   NOT NULL DEFAULT 'INR',
...
-- ---------------------------------------------------------------------
-- 4.8  renewal_batches
--      Groups subscriptions into a single bulk renewal quote.
--      Added: 2026-06-30 (Prisma-first, backfilled here 2026-07-01)
-- ---------------------------------------------------------------------
CREATE TABLE renewal_batches (
    ...
    has_annexure            BOOLEAN         NOT NULL DEFAULT FALSE,
    annexure_data           JSONB,
...
CREATE INDEX idx_rb_org        ON renewal_batches(organization_id);
CREATE INDEX idx_rb_customer   ON renewal_batches(zoho_customer_id);
CREATE INDEX idx_rb_created_at ON renewal_batches(created_at DESC);
...
CREATE TABLE renewal_history (
    ...
    -- Billing currency snapshot for this event (amounts above are in this currency).
    currency                VARCHAR(10)   NOT NULL DEFAULT 'INR',
    exchange_rate           NUMERIC(16,6) NOT NULL DEFAULT 1,

    renewal_status          VARCHAR(30)   NOT NULL DEFAULT 'Quoted',

    -- Live Zoho document statuses (synced on demand via the refresh action)
    zoho_estimate_status    VARCHAR(30),  -- draft|sent|accepted|declined|invoiced|expired
    zoho_invoice_status     VARCHAR(30),  -- draft|sent|paid|partially_paid|overdue|void
    sent_at                 TIMESTAMPTZ,
    ...
    -- Bulk renewal grouping (NULL for individually generated renewals)
    bulk_renewal_batch_id   UUID          REFERENCES renewal_batches(id) ON DELETE SET NULL,
...
CREATE INDEX idx_rh_batch             ON renewal_history(bulk_renewal_batch_id) WHERE bulk_renewal_batch_id IS NOT NULL;


-- ---------------------------------------------------------------------
-- 4.10 lead_conversions
...
CREATE TABLE org_settings (
    ...
    -- Per-organization numbering formats (override global formats if set)
    quote_number_format             VARCHAR(50),
    lead_number_format              VARCHAR(50),
    subscription_number_format      VARCHAR(50),
...
-- =====================================================================
-- 13. CSV IMPORT AUDIT LOG
--     Persists the outcome of every subscriptions bulk CSV import run
--     (price/cost/date/status updates via /api/subscriptions/import-csv),
--     so skipped rows and errors can be reviewed after the response has
--     been dismissed in the UI (added: 01 Jul 2026, TASKS.md item
--     "Add robust logging for CSV import errors/skips").
-- =====================================================================
CREATE TABLE csv_import_logs (
    id                  UUID          PRIMARY KEY DEFAULT gen_random_uuid(),

    import_type         VARCHAR(50)   NOT NULL DEFAULT 'subscriptions_bulk_update',
    file_name           VARCHAR(255),

    total_rows          INTEGER       NOT NULL DEFAULT 0,
    updated_count       INTEGER       NOT NULL DEFAULT 0,  -- bulk-update imports ('subscriptions_bulk_update')
    created_count       INTEGER       NOT NULL DEFAULT 0,  -- new subscriptions created (CSV create + Zoho import)
    enriched_count      INTEGER       NOT NULL DEFAULT 0,  -- existing subs enriched with missing history (no duplicate)
    skipped_count       INTEGER       NOT NULL DEFAULT 0,
    error_count         INTEGER       NOT NULL DEFAULT 0,

    -- Array of { row, id, reason } — rows with no actionable change (blank ID, no fields to update).
    skipped_rows        JSONB,
    -- Array of { row, id, reason } — rows that failed validation or the DB update.
    error_rows          JSONB,
    -- Array of { row, id, reason } — rows imported but flagged (cycle/date-span mismatch, term overlap).
    warning_rows        JSONB,

    created_by          VARCHAR(150),  -- denormalized user identifier/email snapshot (no FK — survives user deletion)
    created_at          TIMESTAMPTZ   NOT NULL DEFAULT NOW()
);

CREATE INDEX idx_csv_import_logs_created ON csv_import_logs(created_at DESC);
CREATE INDEX idx_csv_import_logs_type    ON csv_import_logs(import_type);

COMMENT ON TABLE  csv_import_logs IS 'Append-only audit trail of subscriptions CSV bulk import runs — full skipped/error detail persists after the API response is gone.';
COMMENT ON COLUMN csv_import_logs.skipped_rows IS 'JSONB array of rows that had nothing to update (blank ID column or no recognized fields present).';
COMMENT ON COLUMN csv_import_logs.error_rows IS 'JSONB array of rows that failed validation (bad date/status/number) or the database update (e.g. subscription ID not found).';
...
--
-- Total objects:
--   Tables:    21  (11 core + 5 settings + 4 auth + 1 audit)
--   Indexes:   79+
--   Triggers:  14
--   Sequences: 3
--   Functions: 4
